...
class SupConResNet(nn.Module):
    ...
    def __init__(self, name='r152_1x_sk0', head='mlp', feat_dim=128, pretrained=False):
        super(SupConResNet, self).__init__()
        self.encoder, _ =  get_resnet(*name_to_params(name))
        dim_in = self.encoder.fc.in_features 
        
        if pretrained:
            print(f"Using pre-trained models for {name}!")
            cpt = torch.load(f'/experimentos/pesos/simclr/{name}.pth')
            msg = self.encoder.load_state_dict(cpt['resnet'])
#             self.encoder = self.encoder.net
            print(15*"**")
...
def data_transforms(jitter_strength = 1.0):
    ...
    augmentations['train'] = transforms.Compose(data_transforms)
    ...
    augmentations['val'] = transforms.Compose(val_trans)
    
    augmentations['test'] = transforms.Compose(val_trans)
...
LOAD_PATHS = {'resnet50': '/experimentos/pesos/simclr/simclr_r50_800.pth',
             'r152_1x_sk0': '/experimentos/pesos/simclr/r152_1x_sk0.pth'}    
...
class SupConResNet(nn.Module):
    ...
    def __init__(self, name='resnet50', head='mlp', feat_dim=128, pretrained=False):
        ...
        if pretrained:
            print(15*"**")
            print(f"Loading pre-trained model for {name} -- From path: {LOAD_PATHS[name]}")
            cpt = torch.load(LOAD_PATHS[name])
            
            if name.startswith("r152"):
                msg = self.encoder.load_state_dict(cpt['resnet'])
            else:
